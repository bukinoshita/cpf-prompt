...
const eraseLines = require('./utils/erase-lines')
const { circleDotted } = require('figures')
...
module.exports = function(
  ...
  return new Promise((resolve, reject) => {
    ...
    if (valid) {
      ...
      const _label = label.replace(circleDotted, '✖')
    ...
    async function onData(buffer) {
      ...
      if (suggestion !== '' && !caretOffset && autoCompleteChars.has(data)) {
        ...
        if (validateValue(value)) {
          ...
          const l = chalk.red(label.replace(circleDotted, '✖'))
          eraseLines(1)
